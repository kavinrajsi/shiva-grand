...
export async function sendBookingInquiry(_prevState, formData) {
  ...
  try {
    ...
    const { error: dbError } = await supabase.from("booking_inquiries").insert({
      ...
      notes: data.requests || null,
      source: "book-you-stay",
    });
  ...
  const adminHtml = `
    <div style="font-family:Manrope,Arial,sans-serif;max-width:560px;margin:auto;color:#191c1b;">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tr><td style="padding:6px 0;color:#404942;">Guests</td><td style="padding:6px 0;">${safe.guests}</td></tr>
        <tr><td style="padding:6px 0;color:#404942;">Room type</td><td style="padding:6px 0;">${safe.roomType}</td></tr>
        <tr><td style="padding:6px 0;color:#404942;vertical-align:top;">Special requests</td><td style="padding:6px 0;">${safe.requests}</td></tr>
      </table>
    </div>
  `.trim();
  ...
  const userHtml = `
    <div style="font-family:Manrope,Arial,sans-serif;max-width:560px;margin:auto;color:#191c1b;">
      <h2 style="color:#0d5533;margin:0 0 16px;">Thanks, ${safe.name} — we got your booking request.</h2>
      <p style="line-height:1.6;color:#404942;">
        Your reservation is <strong>not yet confirmed</strong>. A team member will call you shortly to verify availability and finalize the details.
      </p>
      <div style="background:#f2f4f2;padding:16px;border-radius:12px;font-size:14px;color:#191c1b;">
        ...
        <p style="margin:4px 0;"><strong>Guests:</strong> ${safe.guests}</p>
        <p style="margin:4px 0;"><strong>Room type:</strong> ${safe.roomType}</p>
      </div>
  ...
  try {
    const [adminResult, userResult] = await Promise.all([
      resend.emails.send({
        ...
        bcc,
        replyTo: data.email,
        subject: `New booking inquiry — ${data.name} (${data.checkIn} → ${data.checkOut})`,
        html: adminHtml,
      }),
      resend.emails.send({
        from,
        to: data.email,
        subject: "Your booking request is being processed — Shiva Grand",
        html: userHtml,
      }),
    ]);
